    ...
    try {
        ...
        const response = await notion.pages.create({
            ...
            properties: {
                ...
                timestamp: { date: { start: new Date().toISOString() } },  // ensure timestamp is set correctly
    ...
    try {
        ...
        if (keywords.length > 0) {
            filters.push({
                property: 'key_words',
                multi_select: {
                    contains: keywords.join(', '),  // Allow filtering by all keywords
                },
            });
        }

        // Filter by memory area ID if provided
        if (memoryAreaId) {
            filters.push({
...
function generateGroupCode(keywords = []) {
    return keywords.slice(0, 3).join('-').toLowerCase();  // Generate a short group code
}
